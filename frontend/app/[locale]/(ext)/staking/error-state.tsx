...
interface StakingErrorStateProps {
  error?: string;
  onRetry?: () => void;
}

export default function StakingErrorState({
  error,
  onRetry,
}: StakingErrorStateProps) {
        ...
        <div className="min-h-[400px] flex items-center justify-center">
          <div className="max-w-md w-full text-center p-8 rounded-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm border border-zinc-200 dark:border-zinc-800 shadow-xl">
            ...
            <h2 className="text-2xl font-bold text-zinc-900 dark:text-white mb-3">
              {t("error_loading_staking")}
            </h2>
            <p className="text-zinc-600 dark:text-zinc-400 mb-6">
              {t("we_encountered_an_error_while_loading_1")}
            </p>
            {error && (
