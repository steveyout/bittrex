...
interface PoolDetailErrorStateProps {
  error?: string;
  onRetry?: () => void;
}

export default function PoolDetailErrorState({
  error,
  onRetry,
}: PoolDetailErrorStateProps) {
        ...
        <div className="min-h-[400px] flex items-center justify-center">
          <div className="max-w-md w-full text-center p-8 rounded-2xl bg-white/50 dark:bg-zinc-900/50 backdrop-blur-sm border border-zinc-200/50 dark:border-zinc-700/50 shadow-xl">
            ...
            <h2 className="text-2xl font-bold text-zinc-900 dark:text-white mb-3">
              {t("error_loading_pool")}
            </h2>
            <p className="text-zinc-600 dark:text-zinc-400 mb-6">
              {t("we_encountered_an_error_while_loading_4")}
            </p>
            {error && (
